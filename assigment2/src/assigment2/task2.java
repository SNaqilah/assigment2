package assigment2;

import java.util.Scanner;

public class task2 {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		Scanner input = new Scanner (System.in);
	    System.out.println("Asiggment");
	    Scanner sc = new Scanner(System.in);

	    System.out.println("What is your name: ");
	    String name = sc.next();
	    System.out.println("Your age: ");
	    int age = sc.nextInt();
	    System.out.println("Your ic number: ");
	    int ic = sc.nextInt();
	    System.out.println("What is your gender: ");
	    char gender = sc.next().charAt(0);
	    System.out.println("Your phone number: ");
	    int phone = sc.nextInt();
	   
	    

	    System.out.println("\nWELCOME TO CUTIE SHOP.");
	    System.out.println("What product in our shop you want?");
	    String product = "Purfume";
	    System.out.println("\nOur shop sells " + product + " products");

	    System.out.println("What "+ product +"?");
	    System.out.println("\nWe sell Citrus $170");
	    System.out.println("We sell Gourmand for $122");
	    System.out.println("We sell Floral for $385");
	    System.out.println("We sell Fruity for $136");
	    System.out.println("We sell Spicy for $148");
	    System.out.println("We sell Aquatic for $126");
	   

	    System.out.println("\nThere would be some discounts that you will get if you buy our product.");
	    System.out.println("\nWould you like to buy our products? (Y for yes / N for no)");
	    char ans = sc.next().charAt(0);

	int q1=0, q2=0, q3=0, q4=0, q5=0, q6=0;
	double total = 0, price1=0, price2=0, price3=0, price4=0, price5=0, price6=0;

	if(ans == 'Y' || ans == 'y') {
		System.out.println("How much would you buy Citrus ?");
		q1 = sc.nextInt();
		price1 = q1 * 23;
		System.out.println("How much would you buy Gourmand ?");
		q2 = sc.nextInt();
		price2 = q2 * 32;
		System.out.println("How much would you buy Floral ?");
		q3 = sc.nextInt();
		price3 = q3 * 29;
		System.out.println("How much would you buy Fruity ?");
		q4 = sc.nextInt();
		price4 = q4 * 16;
		System.out.println("How much would you buy Spicy ?");
		q5 = sc.nextInt();
		price5 = q5 * 19;
		System.out.println("How much would you buy Aquatic ?");
		q6 = sc.nextInt();
		price6 = q6 * 15;

		total = price1 + price2 + price3 + price4 + price5 + price6;
	}

	else if(ans == 'N' || ans == 'n') {
		System.out.println("\nThank you for visiting our shop ;)");
	}
			
	double discount = 0, dprice = 0;


	if ( total >= 220 ) {
		discount = total*25/100;
		dprice = total - discount;
	}	
	else if ( total > 100 || total < 200 ) {
		discount = total*15/100;
		dprice = total - discount;
	}
	else {
		discount = total*10/100;
		dprice = total - discount;
	}

	System.out.println(" ");

	int t=1;
	do {
		if (t>0)
			System.out.print("*");
		t++;
	} while(t<50);

	System.out.println(" ");

	System.out.println("\nName: " + name);
	System.out.println("Age: " + age);
	System.out.println("Ic number: " + ic);
	System.out.println("Gender: " + gender);
	System.out.println("HP: " + phone);


	System.out.println("\nSAI BEAUTY AND CARE SHOP.");
	System.out.println("\t\t\t\s\s\s\sQuantity\tprice");
	System.out.println("1.\tFace serum:\t\t"+q1+"\t" + price1);
	System.out.println("2.\tDay lotion:\t\t"+q2+"\t" + price2);
	System.out.println("3.\tNight cream:\t\t"+q3+"\t" + price3);
	System.out.println("4.\tSun screen:\t\t"+q4+"\t" + price4);
	System.out.println("5.\tUnder-eye cream:\t"+q5+"\t" + price5);
	System.out.println("6.\tCleanser:\t\t"+q6+"\t" + price6);

	System.out.println(" ");
	int i=1, line=0;
	while (i <50) {
		i = line++;
		System.out.print("-");
	}
	System.out.println(" ");

	System.out.println("\nThe total price is:\t"+ total);
	System.out.println("Your discount will be:\t"+ discount);		
	System.out.println("Your price to be paid:\t"+ dprice);		

	System.out.println(" ");

	for(int e=1; e <50; e++) {
		System.out.print(".");
	}
	System.out.println(" ");

	sc.close();

	}

	}

}
